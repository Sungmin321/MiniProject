...
public class WeightEDDialog extends JDialog {

	private final JPanel contentPanel = new JPanel();

	public static void main(String[] args) {
		try {
			WeightEDDialog dialog = new WeightEDDialog();
	...
	public WeightEDDialog() {
		JTextField weightTF;
		JCheckBox ExerciseCheckbox;
		JCheckBox DrinkingCheckbox;
		setModal(true);
		setSize(450, 300);
				...
				okButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						if (new MemberDAO().hasInout(MemberVo.user.getId())) {
							// 있으면 true -> 그럼 업데이트.
							if (new InsertUpdate().isInOutWeightEDModify(MemberVo.user.getId(), weight, Drinking,
									Exercise)) {
								JOptionPane.showMessageDialog(null, "data correction");
							}
						} else {
							// 없으면 false. 새로 insert 해주고
							if (new InsertUpdate().isInoutInsert(MemberVo.user.getId(), weight, Drinking, Exercise)) {
