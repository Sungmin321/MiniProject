...
public class PersonalTrainingCancel {

	private JFrame frame;

	public PersonalTrainingCancel() {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (now.getFlag().charAt(0) == '0') {
				...
				contents[i][1] = new MemberDAO().getTrainerName(now.getTR_Id()); // 트레이너 이름.
		...
		JComboBox trnamecomboBox = new JComboBox();
		...
		cancelButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (new MemberDAO().hasPtReservation(id, date, tr_id, hour)) {
