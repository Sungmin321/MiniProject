...
public class Customer_Plan {
	...
	public Customer_Plan() {
		...
		StartButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (new MemberDAO().plan_recode_test(MemberVo.user.getId())) { // 데이터가 있다면 트루. 없다면 false
					if (new Insert().plan_recode_Stime_update(MemberVo.user.getId())) {
						JOptionPane.showMessageDialog(null, "시작시간 저장완료");
					} else {
						JOptionPane.showMessageDialog(null, "데이터 저장 실패"); // 
					}
				} else {
					if (new Insert().plan_recode_insert(MemberVo.user.getId())) {
						JOptionPane.showMessageDialog(null, "시작시간 저장완료");
					} else {
		...
		EndButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (new MemberDAO().plan_recode_test(MemberVo.user.getId())) {
					if (new Insert().plan_recode_Etime_update(MemberVo.user.getId())) {
